...
Legacy built-in channels still use their existing independent JSON sections on the transitional app plane. The v1 operator endpoint is different: it is an explicitly selected typed module under `channels.operator`, using `@mono-agent/channel-operator`, mandatory bearer auth, and a loopback-only listener. Standalone operator products are not selected by agent config.
...
| `channel.operator` | config | `channels.operator: {$use:"@mono-agent/channel-operator",listen:{host,port},auth:{token:{$env}},label?}`; explicit selection, loopback-only, auth required; current capabilities are text turns, cancellation, runtime overrides, and health | Operator-chosen token env, commonly `MONO_AGENT_OPERATOR_TOKEN` | [Operator channel](/channels/tui/) | — |
...
| `operator.tui` | cli + code | `mono-agent-tui [--endpoint <url> [--token-env <name>] \| --registry <dir>...] [--agent <id\|label>] [--conversation <id>] [--model <ref>] [--effort <level>]`; `startMonoAgentTui(...)`; no embedded/local responder, replay/config, or self-config | Direct mode defaults to required `MONO_AGENT_OPERATOR_TOKEN`; discovery descriptors name their token env | [Terminal operator](/observability/tui/) | — |
| `operator.web` | config + cli + code | Strict independent `web.config.json`: `configVersion`, `listen`, `auth.token.$env`, `allowInsecureHttp`, `dataDirectory`, `agentRegistries`; `mono-agent-web [config-path]`; source-bound durable text threads, browser-disconnect survival, model/effort, cancel; no uploads/notifications/AskUser UI/PWA/service manager in the first slice | Operator-chosen web bearer env, commonly `MONO_AGENT_WEB_AUTH_TOKEN` | [Web operator](/observability/web-console/) | — |
