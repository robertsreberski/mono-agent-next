...
`TriggerReceipt.status` and `code` are machine authority: `unknown` is distinct
from a definitive rejection, while `reason` is display-only and may be redacted.
...
pass Core's ordinary validation unchanged.
