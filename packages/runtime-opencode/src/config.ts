...
export const runtimeOpenCodeJsonSchema = {
  ...
  properties: {
    ...
    minimumVersion: { type: "string", pattern: "^(0|[1-9]\\d*)\\.(0|[1-9]\\d*)\\.(0|[1-9]\\d*)$", default: "1.15.0" },
...
export function parseRuntimeOpenCodeConfig(input: unknown): RuntimeOpenCodeConfig {
  ...
  const minimumVersion = string(value.minimumVersion, "1.15.0", "runtime-opencode config.minimumVersion");
  if (!VERSION.test(minimumVersion)) throw new TypeError("runtime-opencode config.minimumVersion must be stable semver");
  if (value.pure !== undefined && typeof value.pure !== "boolean") throw new TypeError("runtime-opencode config.pure must be a boolean");
  return {
