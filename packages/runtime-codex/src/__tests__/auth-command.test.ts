...
function authCommand(runtime: Runtime): ModuleCommand {
  const command = runtime.commands?.find(({ name }) => name === "codex:auth");
  if (command === undefined) throw new Error("codex:auth command is missing");
  return command;
}

describe("codex:auth", () => {
  it("is available before start and reports config versus ambient auth without secrets", async () => {
    const secret = `codex-secret-${"x".repeat(100_000)}`;
    const configured = await create({ auth: { apiKey: secret } });
    ...
    expect(serialized).not.toContain("codex-secret");
  ...
  it("rejects accessor, oversized, and aborted input with bounded fixed errors", async () => {
    ...
    expect(() => command.run(accessor, { logger, signal })).toThrow(
      "runtime-codex auth input must contain only action: status, models, or login",
    );
    expect(getterCalled).toBe(false);
    expect(() => command.run({ action: "secret".repeat(100_000) }, { logger, signal }))
      .toThrow("runtime-codex auth input must contain only action: status, models, or login");
