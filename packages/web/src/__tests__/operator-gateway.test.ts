...
describe("web operator gateway", () => {
  it("discovers only replayable proactive conversations through the identity-bound shared client", async () => {
    ...
    const fetchImpl: typeof fetch = async (input) => {
      ...
      if (
        url.endsWith("/v1/conversations/opaque-external-cron/replay")
        || url.endsWith("/v1/conversations/opaque-external-webhook/replay")
      ) {
  ...
  it("uses authoritative info and the shared override policy before forwarding overrides", async () => {
    ...
    // `{ runtime, id }` is atomic: a model advertised only for the default
    // runtime must not be reachable by naming a different runtime.
    await expect(gateway.runTurn(turn({ runtime: "pi-secondary", model: "approved:model" })))
      .rejects.toMatchObject({ code: "unknown_model" });
    expect(forwarded).toHaveLength(0);

    await expect(gateway.runTurn(turn({ runtime: "pi-secondary", model: "secondary:model", effort: "high" }))).resolves.toBeUndefined();
    expect(forwarded).toMatchObject([{ runtime: "pi-secondary", model: "secondary:model", effort: "high" }]);
...
function operatorInfo(runtimeOverrides: boolean, startedAt: string): OperatorInfo {
  return {
    ...
    models: [
      ...
      { runtime: "pi-secondary", id: "secondary:model", efforts: ["low", "high"] },
    ],
  };
}
