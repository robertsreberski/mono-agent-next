...
export interface RunHistoryReader {
  listRuns(cursor: string | undefined, signal: AbortSignal): Promise<AgentRunHistoryPage>;
  readRun(runId: string, signal: AbortSignal): Promise<AgentRunRecord | undefined>;
}
type Cursor =
...
export function createRunHistoryToolContribution(
  ...
  return Object.freeze({
    ...
    bind(context: ModuleToolTurnContext) {
      const cursors = new Map<string, Cursor>();
      return Object.freeze({
        async execute(input: JsonValue, call: ModuleToolCallContext) {
          const signal = AbortSignal.any([context.signal, call.signal]);
          throwIfAborted(signal);
          const parsed = parseInput(input, cursors);
          if ("error" in parsed) return result(parsed.action, { error: parsed.error }, true);
          if (parsed.action === "inspect") {
            const record = await reader.readRun(parsed.runId, signal);
            throwIfAborted(signal);
            if (!eligible(record?.summary, context)) {
              return result("inspect", { error: "run_not_available" }, true);
            }
          ...
          const runs = await collectEligibleRuns(reader, context, signal);
          const selected = parsed.action === "search"
            ? await searchRuns(runs, parsed.query, reader, context, signal)
            : runs;
...
async function collectEligibleRuns(
  ...
  for (let page = 0; page < SOURCE_PAGE_LIMIT && runs.length < SOURCE_RUN_LIMIT; page += 1) {
    const result = await reader.listRuns(cursor, signal);
    throwIfAborted(signal);
    for (const run of result.runs) if (eligible(run, context)) runs.push(run);
...
async function searchRuns(
  ...
  for (const summary of runs.slice(0, SEARCH_READ_LIMIT)) {
    const record = await reader.readRun(summary.runId, signal);
    throwIfAborted(signal);
    if (!eligible(record?.summary, context)) continue;
    const text = [
      summary.runId,
      summary.status,
      summary.startedAt,
      summary.endedAt ?? "",
      summary.failureCode ?? "",
      ...record.transcript.flatMap(searchableText),
    ].join("\n").toLocaleLowerCase();
...
function eligible(
  ...
  return summary !== undefined
    && summary.conversationId === context.conversationId
...
function projectTimeline(record: AgentRunRecord): readonly JsonObject[] {
  return [
    ...
    ...record.transcript.map((entry) => {
      ...
      return sanitize({
        kind: "transcript",
        role: entry.kind === "message" ? entry.role : "interaction",
        recordedAt: entry.recordedAt,
...
function sanitize(value: unknown, key = "", depth = 0): JsonValue {
  ...
  for (const [field, entry] of Object.entries(value).slice(0, 100)) {
    output[field] = sanitize(entry, field, depth + 1);
  }
  return output;
}
function result(action: string, data: Record<string, JsonValue>, isError = false) {
  return {
    content: [{
      type: "json" as const,
      value: sanitize({
        notice: RUN_HISTORY_UNTRUSTED_NOTICE,
        action,
        ...data,
      }) as JsonObject,
    }],
...
function remember(cursors: Map<string, Cursor>, value: Cursor): string {
  while (cursors.size >= CURSOR_LIMIT) {
    cursors.delete(cursors.keys().next().value as string);
  }
...
function boundedText(value: string): string {
  ...
  return bytes.byteLength <= TEXT_LIMIT
    ? value
    : `${bytes.subarray(0, TEXT_LIMIT).toString("utf8")}…`;
}
...
function throwIfAborted(signal: AbortSignal): void {
  if (signal.aborted) {
    throw signal.reason instanceof Error ? signal.reason : new Error("RunHistory aborted");
  }
}
