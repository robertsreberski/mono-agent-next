...
Tier: `core`
Catalog responsibility: Serves one selected agent through the authenticated loopback operator protocol.
...
translate accepted turns into the typed channel host contract, and cancel core
dispatch when an operator disconnects or explicitly cancels a conversation.
...
4. Channel reply events become only shared operator NDJSON frames; the terminal frame carries the authoritative result.
5. Client disconnect, explicit cancellation, drain, and stop abort the exact dispatch signal.
...
| `server.ts` | Authenticated HTTP routes, bounded JSON, NDJSON streaming, cancellation, health, and shutdown. |
...
It does not decode operator state, reduce product actions, persist product
conversations, expose a non-loopback listener, terminate TLS, select models, or
keep running after its host stops. TUI and web remain independently installed
products.
